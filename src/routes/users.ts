...
import { authenticate, authorize, auditLog, authorizeResourceOwnership } from '../middleware/auth';
import { 
  createUserSchema, 
  updateUserSchema, 
  paginationSchema, 
...
router.get(
  '/:id',
  authorizeResourceOwnership('user'),
  auditLog('read', 'user'),
  async (req: Request, res: Response): Promise<void> => {
    try {
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { error: idError, value: userId } = objectIdSchema.validate(req.params.id);
      if (idError) {
        res.status(400).json({
          success: false,
          message: 'Invalid user ID'
      ...
      ['firstName', 'lastName', 'isActive'].forEach(field => {
        if (value[field] !== undefined && value[field] !== user[field as keyof typeof user]) {
          originalValues[field] = user[field as keyof typeof user];
          changes[field] = { from: user[field as keyof typeof user], to: value[field] };
        }
      });
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const rolesValidation = updateUserSchema.extract('roles').validate(req.body.roles);
